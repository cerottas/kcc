"""Ray Jobs adapter for the stable runtime."""
...
class StableRayJobsRest(RayJobsRest):
    def submit_once(
        self,
        address: str,
        submission_id: str,
        ...
        *,
        ...
        if existing is not None:
            owner = existing.get("metadata", {})
            if not isinstance(owner, Mapping) or any(owner.get(key) != value for key, value in metadata.items()):
                raise RayJobsRestError(409, "existing Ray job ownership metadata differs")
            return submission_id
        payload = {
            "entrypoint": "python -m kcc_training.runtime.coordinator_stable --spec /etc/kcc/run/run.json",
